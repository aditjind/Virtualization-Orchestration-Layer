...
def volumeAttach():
    ...
    imageName = str(VOLUME_DICT[volumeId]['name'])
    conn = libvirt.open("qemu+ssh://"+pm_chosen+"/system")
...
</domain>""" % (i, Vm_name, str(int(vm_ram)*1000), str(vm_cpu), "'/home/"+user_name+"/"+vm_image_name+"'"))
        xml = """<domain type='qemu' id='%s'>
  	            <name>%s</name>
  	            <memory>%s</memory>
  	            <currentMemory>512000</currentMemory>
...
def make_image_list():
    ...
    #images = os.listdir("/home/aditya/images")
